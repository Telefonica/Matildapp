...
class CustomModule(Module):
    ...
    def run_module(self):
       ...
       #read contract path
        ...
        bytecode = compiled["contracts"][self.args["contract"]][k[0]]["evm"]["bytecode"]["object"]
        opcode = compiled["contracts"][self.args["contract"]][k[0]]["evm"]["bytecode"]["opcodes"]
        ...
        contract_address = Connect.get_instance().deploy_contract(abi,bytecode,opcode,self.args["address"],self.args["pkey"])
        #name = DataStore.get_instance().random_name()
        if not DataStore.get_instance().is_key("contract",self.args['contract']):
            DataStore.get_instance().create_name("contract",self.args['contract'])
            DataStore.get_instance().add_value("contract",self.args['contract'],'address',contract_address)
            DataStore.get_instance().add_value("contract",self.args['contract'],'abi',abi)
            DataStore.get_instance().add_value("contract",self.args['contract'],'opcode',opcode)
            DataStore.get_instance().add_value("contract",self.args['contract'],'bytecode',bytecode)
            DataStore.get_instance().add_value("contract",self.args['contract'],'source',self.args['contract'])
            DataStore.get_instance().add_value("contract",self.args['contract'],'source_content',contract_content)
        
        if not DataStore.get_instance().is_key("wallet",self.args['contract']):
            DataStore.get_instance().create_name("wallet",self.args['contract'])
            DataStore.get_instance().add_value("wallet",self.args['contract'],'address',self.args['address'])
            DataStore.get_instance().add_value("wallet",self.args['contract'],'pkey',self.args['pkey'])
